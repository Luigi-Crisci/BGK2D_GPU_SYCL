...
struct usm_buffer {
    ...
    void update_host() {
        m_q->wait();
        if constexpr(kind == sycl::usm::alloc::device) { m_q->copy(m_device_ptr, m_host_ptr, m_size).wait(); }
    }
    ...
    void update_device() {
        m_q->wait();
        if constexpr(kind == sycl::usm::alloc::device) { m_q->copy(m_host_ptr, m_device_ptr, m_size).wait(); }
    }
